...
export class AddunitsComponent implements OnInit {
  ...
  cambio2() {
    if (this.id_sector_search == 'value') {
      ...
      this.ListadoUnidades = this.ListadoConjuntosSelect2[this.id_sector_search]['units'];
  ...
  saveChanges() {
    ...
    this.addUnitservice.addUnits(formData2, formData, this.keysession, this.customer_id, this.meeting_id);
  ...
  enableLogin() {
    ...
    this.httpClient.post(this.config.endpoint3 + 'CustomerRegistrationServices/updateUserSignInStatus', formData2).subscribe((user) => {
    });
  }
