...
export class ListVoteByResidentialComponent implements OnInit {
  ...
  ngOnInit() {
    this.httpClient
      .get(
        this._env.ENDPOINT_PRIMARY +
          this._env.APP_PREREGISTRO +
          'UtilServices/getVotesByMeeting?key=' +
  ...
  Inform(voting_id: any, name: any) {
    this.httpClient
      .get(
        this._env.ENDPOINT_PRIMARY +
          this._env.GESTOR_V2 +
          'api/voting/getVotingReportByHeaderExcel/' +
          this.keysession +
          '/' +
          voting_id +
          '/' +
          this.with_cutomer_name
      )
      .subscribe((resp: any) => {
        var base64decode = decodeURIComponent(
          atob(resp['content'])
            .split('')
            .map(function (c) {
        ...
        var blob = new Blob([base64decode], {
    ...
    if (status == '1') {
      ...
      this.httpClient
        .get(
          this._env.ENDPOINT_PRIMARY +
            this._env.APP_PREREGISTRO +
            'VotingServices/getActiveVoteOptionByMeeting?key=' +
            this._env.SECRET_KEY +
            '&meeting_id=' +
            this.meeting_id
        )
        .subscribe((resp: any) => {
          if (resp['success'] == true) {
            swal
              .fire({
                title:
                  'Hay otra votacion activa desea desactivar esa y activar la seleccionada?',
              ...
              .then((result) => {
                if (result['value'] == true) {
                  ...
                  formData2.append(
                    'request_accepted',
                    resp['content']['request_accepted']
                  );
                  formData2.append('status_id', '0');
                  this.createAnswerService.editAnswer(
                    formData2,
                    this.residential_id,
                    '0',
                    '0',
      ...
      this.httpClient
        .get(
          this._env.ENDPOINT_PRIMARY +
            this._env.APP_PREREGISTRO +
            'VotingServices/getVotingOptionResults?key=' +
            this._env.SECRET_KEY +
            '&user_id=' +
            this.user_id +
            '&header_id=' +
        ...
        .subscribe((resp: any) => {
          ...
          this.options_save = JSON.stringify(
            resp['content']['vote']['options']
          );
      ...
      .then((result) => {
        if (result['value'] == true) {
          ...
          this.httpClient
            .post(
              this._env.ENDPOINT_PRIMARY +
                this._env.APP_MANAGEMENT +
                'ApiVoting/deleteVotingQuestion',
              formData2
            )
            .subscribe((resp3) => {});
        }
  ...
  InformAttendanceRealTime() {
    this.httpClient
      .get(
        this._env.ENDPOINT_PRIMARY +
          this._env.GESTOR_V2 +
          'api/reports/getSessionConfirmation/' +
          this.keysession +
          '/' +
          this.meeting_id
      )
  ...
  InformChat() {
    ...
    this.httpClient
      .get(
        this._env.ENDPOINT_PRIMARY +
          this._env.GESTOR_V2 +
          'api/chat/getMessagesFromMeeting/' +
          this.keysession +
          '/' +
          this.meeting_id +
          '/0'
      ...
      .subscribe((resp2: any) => {
        if (resp2['success']) {
          for (
            let index = 0;
            index < resp2['content']['messages'].length;
            index++
          ) {
            if (resp2['content']['messages'][index]['user_id']) {
              const chats = new Chats(
                ...
                this.transformTimeZone(
                  resp2['content']['messages'][index]['created_at']
                ),
        ...
        setTimeout(() => {
          ...
          pdf.html(source, {
            margin: [margins.left, margins.top],
            width: margins.width,
            callback: function () {
